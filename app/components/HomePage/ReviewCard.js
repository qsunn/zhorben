...
export const ReviewCard = ({ employee, index, currentSlide }) => {
    ...
    return (
        <div className={`relative flex box1 flex-col my-5 items-start gap-5 p-10 pl-5 ${isActiveSlide ? '!bg-right-top !bg-auto !bg-no-repeat !bg-[url("./images/bg_squares_reviews.png")]' : '!bg-[#F6F6F6]'}`}>
            <div className={`h-[1.125rem] w-12 ${s.image} ${employee.logo}`} />
            <div className="text-yellow">{employee.logoText}</div>
            <div className="text-secondary-light dark:text-secondary-dark">{employee.about}</div>
